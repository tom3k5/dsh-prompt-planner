...
// so the tool is registered exactly once the LLM, tool registries and system
// prompt assembly are up.
...
export function apply(ctx) {
  ...
  const tool = defineTool({
    ...
    // 900 s (15 min): limit musi obejmować nie tylko pracę LLM (diagnoza +
    // plan), ale też oczekiwanie na odpowiedzi człowieka w fazie pytań
    // uszczegóławiających (userQuestions.ask). 300 s okazało się za krótkie,
    // gdy użytkownik nie odpowiada od razu — TOOL_TIMEOUT ubijał całe wywołanie.
    timeoutMs: 900000,
    parameters: {
      prompt: {
        type: 'string',
  ...
  // ── automatyczne wywołanie: sekcja system prompt mówi modelowi, by sam
  // decydował o użyciu prompt_plan przed złożonymi/złożonymi-wieloetapowymi
  // zadaniami (wzorzec jak dsh-tool-cordis / tool-ralph). Order 2550 = tuż po
  // TOOL_CORDIS (2500), przed TOOL_WORKFLOW (2600) — grupa planistyczna.
  ctx.systemPrompt.section({
    name: 'tool:prompt-plan',
    order: 2550,
    text: 'Use the `prompt_plan` tool before executing a complex or multi-part request: it analyzes the prompt, asks clarifying questions when the intent is ambiguous, improves the prompt, decomposes it into atomic tasks with difficulty ratings (1-5), picks the best available model + reasoning effort for each task from the live catalog, and returns an execution plan with token estimates. Call it proactively for complex, multi-part, or long-running objectives; do not call it for routine single-turn work.',
  })
