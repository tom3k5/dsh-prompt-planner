...
export function apply(ctx) {
  ...
  // ── heurystyka tokenów spójna z dsh-token-meter (~4 znaki/token) ──
  ...
  const tool = defineTool({
    ...
    async execute(args, exec) {
      ...
      const clarification = { asked: false, count: 0, answers: [], skippedReason: null }
      ...
      if (clarification.answers.length > 0) {
        ...
        context.push('User answers to clarification questions:\n' + lines.join('\n'))
      }

      const plan = await streamJson(context.join('\n\n'), route, exec.signal, planSystem(), [
      ...
      return {
        ...
        tasks: ordered,
        tokenEstimate: {
          heuristic: '~4 chars per token + structural overhead (same as dsh-token-meter)',
